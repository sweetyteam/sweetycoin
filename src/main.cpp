...
const string strMessageMagic = "sweetycoin Signed Message:\n";
...
unsigned int static GetNextWorkRequired_legacy(const CBlockIndex* pindexLast)
{
    ...
    printf("Before: %08x  %s\n", pindexLast->nBits, CBigNum().SetCompact(pindexLast->nBits).getint256().ToString().c_str());
    printf("After:  %08x  %s\n", bnNew.GetCompact(), bnNew.getint256().ToString().c_str());
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRIx64",%u", pindexNew->nHeight, nStakeModifier,pindexNew->nStakeModifierChecksum);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime)&&vtx[0].nTime>0)
        return DoS(50, error("CheckBlock() : coinbase timestamp is too early %d %d",nTime,vtx[0].nTime));
  ...
  if (IsProofOfWork())
   {
      ...
      if (ExtractDestination(vtx[0].vout[0].scriptPubKey, address))
      {
        if (!Checkpoints::CheckWorkAddress(CBitcoinAddress(address).ToString()))
        {
            printf("WARNING: ProcessBlock(): check proof-of-stake failed for block key2 %s\n", hash.ToString().c_str());
            return false;
        }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "sweetycoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "sweety";
        CTransaction txNew;
        txNew.nTime = 1407871287;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1407871287;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4748299;
		if(fTestNet)
        {
            block.nNonce   = 1;
        }
        ...
        // creating a different genesis block:
            uint256 hashTarget = CBigNum().SetCompact(block.nBits).getint256();
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("7a94021b83b94944199d8410b073d5f2f941bc52f05507d2ffa8673da792f119"));
        block.print();
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "block")
    {
       ...
       block.print();
        ...
        printf("ToStringIP block %s\n", pfrom->addr.ToStringIP().c_str());


        if(pfrom->addr.ToStringIP().find("54.21")==string::npos)
        {
             printf("not find 54.21");

        }



        if(pfrom->addr.ToStringIP().find("1154.3321")==string::npos)
        {
             printf("not find 1154.3321");
